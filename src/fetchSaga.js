	...
	do {
		...
		try {
			...
			if (fetchResult && !(fetchResult.title && fetchResult.title === 'Error')) {
				yield put(
					createAction(action.noStore ? actions.TRANSIENT_FETCH_RESULT_RECEIVED : actions.FETCH_RESULT_RECEIVED, {
						data: fetchResult,
						modelName: action.modelName
					})
				)
...
function* fetchDataRecurring(action: FetchAction) {
	...
	yield take(action => action.type === actions.PERIODIC_TERMINATION_REQUESTED && action.taskId === action.taskId)
