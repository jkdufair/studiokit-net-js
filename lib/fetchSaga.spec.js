...
describe('fetchData', function () {
	...
	test('should add oauth token to header if it exists', function () {
		...
		expect(gen.next().value).toEqual((0, _effects.race)({
			...
			timedOut: (0, _effects.call)(_reduxSaga.delay, 3000)
		}));
...
describe('fetchDataLoop', function () {
	test('should fetch repeatedly until terminated', function () {
		...
		expect(gen.return().value).toEqual((0, _effects.put)((0, _actions.createAction)(_actions2.default.PERIODIC_TERMINATION_SUCCEEDED, {
			modelName: 'foo',
			period: 1000
		})));
		expect(gen.next().done).toEqual(true);
...
describe('fetchSaga', function () {
	...
	test('should set up all takes', function () {
		...
		expect(gen.next().value).toEqual((0, _effects.takeLatest)('auth/GET_TOKEN_SUCCEEDED', interceptOauthToken));
		expect(gen.next().value).toEqual((0, _effects.takeLatest)('auth/TOKEN_REFRESH_SUCCEEDED', interceptOauthToken));
	});
