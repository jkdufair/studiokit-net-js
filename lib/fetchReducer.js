...
function fetchReducer() {
	...
	var leafNode = (0, _utilities2.default)(newObject, action.modelName);
	switch (action.type) {
		case _actions2.default.FETCH_REQUESTED:
			leafNode.isFetching = true;
			leafNode.hasError = false;
			leafNode.timedOut = false;
			return Object.assign({}, (0, _immutable.fromJS)(state).deleteIn('fetchedAt').toJS(), newObject);
		...
		case _actions2.default.KEY_REMOVAL_REQUESTED:
			return (0, _immutable.fromJS)(state).deleteIn(action.modelName.split('.')).toJS();
