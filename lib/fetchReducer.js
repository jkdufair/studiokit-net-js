...
function fetchReducer() {
	...
	var newState = {};
	_get__('createNestedObject')(newState, modelName);
	// leafNode is a reference to the model at the (potentially) nested path referenced by modelName
	var leafNode = _get__('byString')(newState, modelName);

	switch (action.type) {
		case _get__('actions').FETCH_REQUESTED:
			leafNode.isFetching = true;
			leafNode.hasError = false;
			leafNode.timedOut = false;
			return _get__('fromJS')(state).mergeDeep(newState).toJS();

		case _get__('actions').FETCH_RESULT_RECEIVED:
			...
			path.push('data');
			return _get__('fromJS')(state).deleteIn(path).mergeDeep(newState).toJS();

		case _get__('actions').FETCH_FAILED:
			leafNode.isFetching = false;
			leafNode.hasError = true;
			leafNode.timedOut = false;
			return _get__('fromJS')(state).mergeDeep(newState).toJS();

		case _get__('actions').FETCH_TIMED_OUT:
			leafNode.isFetching = false;
			leafNode.hasError = true;
			leafNode.timedOut = true;
			return _get__('fromJS')(state).mergeDeep(newState).toJS();

		case _get__('actions').KEY_REMOVAL_REQUESTED:
			return _get__('fromJS')(state).deleteIn(modelName.split('.')).toJS();
...
var _typeOfOriginalExport = typeof fetchReducer === 'undefined' ? 'undefined' : _typeof(fetchReducer);

function addNonEnumerableProperty(name, value) {
	Object.defineProperty(fetchReducer, name, {
		value: value,
		enumerable: false,
		configurable: true
	});
...
if ((_typeOfOriginalExport === 'object' || _typeOfOriginalExport === 'function') && Object.isExtensible(fetchReducer)) {
